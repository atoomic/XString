...
----------------------------------------------------------------------
    ...
    ppport.h.new -- Perl/Pollution/Portability Version 3.73
    ...
    Use 'perldoc ppport.h.new' to view the documentation below.
...
=head1 SYNOPSIS
  ...
  perl ppport.h.new [options] [source files]
...
This version of F<ppport.h.new> is designed to support operation with Perl
...
Display the version of F<ppport.h.new>.
...
Tell F<ppport.h.new> to check for compatibility with the given
Perl version. The default is to check for compatibility with Perl
version 5.003_07. You can use this option to reduce the output
of F<ppport.h.new> if you intend to be backward compatible only
down to a certain Perl version.

=head2 --cplusplus

Usually, F<ppport.h.new> will detect C++ style comments and
replace them with C style comments for portability reasons.
Using this option instructs F<ppport.h.new> to leave C++
comments untouched.
...
Strip all script and documentation functionality from F<ppport.h.new>.
This reduces the size of F<ppport.h.new> dramatically and may be useful
if you want to include F<ppport.h.new> in smaller modules without
increasing their distribution size too much.

The stripped F<ppport.h.new> will have a C<--unstrip> option that allows
...
F<ppport.h.new>. Also lists if it must be explicitly requested,
if it has dependencies, and if there are hints or warnings for it.

=head2 --list-unsupported

Lists the API elements that are known not to be FULLY supported by F<ppport.h.new>,
...
version of Perl that F<ppport.h.new> supports at all.
...
earlier Perl releases. Use
    ...
    perl ppport.h.new --list-provided

to see which API elements are provided by ppport.h.new.
...
also deprecated. You can safely use the supported API, as F<ppport.h.new>
will provide wrappers for older Perl versions.

=item *

Although the purpose of F<ppport.h.new> is to keep you from having to concern
...
more C<#define>s in your source code before the inclusion of F<ppport.h.new>.
...
macro. Just C<#define> the macro before including C<ppport.h.new>:

    #define DPPP_NAMESPACE MyOwnNamespace_
    #include "ppport.h.new"
...
F<ppport.h.new> on your source code. See the next section for
details.

=head1 EXAMPLES

To verify whether F<ppport.h.new> is needed for your module, whether you
should make any changes to your code, and whether any special defines
should be used, F<ppport.h.new> can be run as a Perl script to check your
...
use something like this:
    ...
    perl ppport.h.new --compat-version=5.6.0 --cplusplus --patch=test.diff

If you only want your code to be scanned without any suggestions
for changes, use:

    perl ppport.h.new --nochanges

You can specify a different C<diff> program or options, using
the C<--diff> option:

    perl ppport.h.new --diff='diff -C 10'

This would output context diffs with 10 lines of context.

If you want to create patched copies of your files instead, use:

    perl ppport.h.new --copy=.new

To display portability information for the C<newSVpvn> function,
use:

    perl ppport.h.new --api-info=newSVpvn

Since the argument to C<--api-info> can be a regular expression,
you can use

    perl ppport.h.new --api-info=/_nomg$/

to display portability information for all C<_nomg> functions or

    perl ppport.h.new --api-info=/./
...
If this version of F<ppport.h.new> is causing failure during
the compilation of this module, please check if newer versions
of either this module or C<Devel::PPPort> are available on CPAN
before sending a bug report.

If F<ppport.h.new> was generated using the latest version of
...
# These are tools that must be included in ppport.h.new.  It doesn't work if given
# a .pl suffix.
#
 ...
 * 5.005 perls, you can use aTHXR (supplied by ppport.h.new), which will
...
/* Hint: isPSXSPC, isPSXSPC_A, isPSXSPC_L1, isPSXSPC_utf8_safe
    ...
    ppport.h.new SPACE version does match VT in all perl releases.  Since VT's are
    extremely rarely found in real-life files, this difference effectively
    doesn't matter */

/* Hint: isSPACE, isSPACE_A, isSPACE_L1, isSPACE_utf8_safe
    Until Perl 5.18, this did not match the vertical tab (VT).  The ppport.h.new
...
/* Hint: SvPVbyte
 * Does not work in perl-5.6.1, ppport.h.new implements a version
